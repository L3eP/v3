...
// POST /api/references — Tambah reference baru (semua role)
router.post('/api/references', isAuthenticated, asyncHandler(async (req, res) => {
  ...
  const validTypes = ['aktifitas', 'sub_node', 'odc', 'odp', 'olt', 'onu', 'priority'];
  ...
  const [result] = await db.query(
    ...
    [type, label, group_name || null, parent_port || null, req.body.latitude || null, req.body.longitude || null, sort_order || 0]
  );
...
// PUT /api/references/:id — Edit reference (semua role)
router.put('/api/references/:id', isAuthenticated, asyncHandler(async (req, res) => {
...
// DELETE /api/references/:id — Hapus reference (semua role)
router.delete('/api/references/:id', isAuthenticated, asyncHandler(async (req, res) => {
  const id = parseInt(req.params.id);
