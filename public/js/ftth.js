document.addEventListener('DOMContentLoaded', async () => {
  ...
  function renderPanel(type) {
    ...
    container.innerHTML = items.map(i =>
      `<div class="ftth-item">
        <div class="ftth-label">${i.lat&&i.lng?'📍 ':''}<span>${esc(i.label)}</span>${i.group?` <span class="ftth-group">${esc(i.group)}</span>`:''}</div>
        <div class="ftth-actions">
          <button class="ftth-edit" onclick="editItem('${type}',${i.id},'${esc(i.label)}','${esc(i.group||'')}','${i.lat||''}','${i.lng||''}')"><i class="fas fa-edit"></i></button>
  ...
  window.editItem = function(type, id, label, group, lat, lng) {
    editData = { type, id, label, group, lat, lng };
  ...
  document.getElementById('saveEditBtn').onclick = async () => {
    ...
    if (TYPE_CONFIG[editData.type].hasGroup) body.group_name = document.getElementById('editGroup').value.trim() || undefined;
    ...
    try {
      const r = await fetch(`/api/references/${editData.id}`, { method:'PUT', headers:{'Content-Type':'application/json'}, body:JSON.stringify(body) });
  ...
  document.getElementById('confirmDelBtn').onclick = async () => {
    ...
    try {
      const r = await fetch(`/api/references/${deleteData.id}`, { method:'DELETE' });
