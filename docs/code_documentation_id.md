...
- **`GET /api/ftth/available-ports?type=&parent=`** — authenticated. **Harus dideklarasikan sebelum `/:id`.** Port terpakai = anak dengan `group_name = parent` **dan `type` yang cocok** (filter type penting — tanpanya, ODC dan ODP yang berbagi `group_name` akan saling mengunci port). Untuk semua tipe induk (OLT, ODC, ODP), anak mulai dari Port 1 — `total_ports` hanya menghitung port yang bisa dipakai anak; koneksi uplink device ke induknya sendiri adalah port fisik terpisah yang tidak pernah dimodelkan atau direservasi di sini (bug off-by-one diperbaiki 2026-09-09 — ODC/ODP dulu salah mereservasi Port 1, kehilangan satu slot usable per device).
...
### 9.8 Alokasi port
`GET /api/ftth/available-ports` — port terpakai = anak dengan `group_name` **dan `type`** yang cocok; anak dari semua tipe induk mulai dari Port 1 — `total_ports` murni hitungan port keluar, port uplink milik induk sendiri tidak pernah ditrack di rentang ini. Konflik ONU via `checkOnuConflicts()`; ODC/ODP via `WHERE group_name=? AND type=? AND parent_port=?` inline.
